...
export const ResumeScoreChecker: React.FC<ResumeScoreCheckerProps> = ({
  ...
  const analyzeResume = useCallback(async () => {
    ...
    // 3. Check for free trial availability (lite_check plan)
    const liteCheckPlan = paymentService.getPlanById('lite_check');
    // A free trial is available if the user doesn't have an active subscription OR their current plan is NOT 'lite_check'
    ...
    if (hasScoreCheckCredits) {
      ...
      _analyzeResumeInternal(); // Proceed with analysis
    }
    // Scenario 2: No credits, but free trial is available and not used
    else if (hasFreeTrialAvailable) {
      onShowAlert(
        'Activate Free Trial',
        'You can activate a free trial to get 2 Resume Score Checks. Would you like to proceed?',
        'info',
        'Activate Free Trial',
        async () => {
          try {
            await paymentService.activateFreeTrial(user.id);
            await refreshUserSubscription(); // Refresh App.tsx state
            onShowAlert('Trial Activated!', 'Your free trial has been activated. You now have 2 score checks.', 'success');
            _analyzeResumeInternal(); // Proceed with analysis after trial activation
          } catch (error: any) {
            onShowAlert('Trial Activation Failed', error.message || 'Failed to activate free trial. Please try again.', 'error');
          }
        }
      );
    }
    // Scenario 3: No credits, and no free trial available (or already used)
    else {
      onShowAlert(
        'No Credits Available',
        'You do not have any Resume Score Check credits. Please purchase a plan to continue.',
        'warning',
        'Upgrade Plan',
                       ...
                       <button
                         ...
                         className={`p-6 rounded-xl border-2 transition-all duration-300 text-left ${
                           scoringMode === 'jd_based'
                             ...
                             : 'border-gray-200 hover:border-blue-300 hover:bg-blue-50 dark:border-dark-300 dark:hover:border-neon-cyan-400 dark:hover:bg-dark-200'
                         }`}
                       >
